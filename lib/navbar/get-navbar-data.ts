...
export async function getNavbarData(supabase: SupabaseLike): Promise<NavbarData> {
  ...
  if (!user) {
    return { isLoggedIn: false, userName: "Mi cuenta", isAdmin: false, isSuperAdmin: false, adminHref: undefined, unreadMessagesCount: 0, unreadNotificationsCount: 0, notifications: [], currentUserId: undefined };
  }
  ...
  const [{ data: profile }, { data: roles }, { data: conversations }, { data: notifications }, { count: unreadNotificationsCount }] = await Promise.all([
    ...
    supabase.from("notifications").select("id, user_id, kind, title, body, href, metadata, read_at, created_at").eq("user_id", user.id).order("created_at", { ascending: false }).limit(20),
  ...
  if (conversationIds.length > 0) {
    const { data: unreadMessages } = await supabase.from("messages").select("id").in("conversation_id", conversationIds).neq("sender_id", user.id).is("read_at", null);
