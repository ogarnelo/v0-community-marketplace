...
export default async function AccountActivityPage() {
  ...
  const listingIds = Array.from(
    new Set([
      ...
      ...allDonations.map((request) => request.listing_id).filter((value): value is string => typeof value === "string" && value.length > 0),
    ])
  ) as string[];
  ...
  const profileIds = Array.from(
    new Set(
      [
        ...
        ...allDonations.map((request) => request.requester_id).filter((value): value is string => typeof value === "string" && value.length > 0),
      ].filter(Boolean)
    )
  ) as string[];
