...
export default async function ConversationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!typedConversation || hiddenConversationIds.has(typedConversation.id) || (typedConversation.buyer_id !== user.id && typedConversation.seller_id !== user.id)) notFound();
  ...
  for (const message of unreadMessages || []) unreadCountMap.set(message.conversation_id, (unreadCountMap.get(message.conversation_id) || 0) + 1);
  ...
  const conversationSummaries: ConversationSummary[] = safeConversations.map((item) => {
    ...
    const latestMessageBody = getOfferChatPreview(latestMessage?.body?.trim()) || getDonationChatPreview(latestMessage?.body?.trim()) || (latestMessage?.attachment_name ? `📎 ${latestMessage.attachment_name}` : "Sin mensajes todavía");
    return { id: item.id, otherName: otherProfile?.business_name || (otherProfile?.full_name && otherProfile.full_name.trim().length > 0 ? otherProfile.full_name.trim() : "Usuario"), listingTitle: listing?.title || "Anuncio", latestMessageBody, latestMessageCreatedAt: latestMessage?.created_at || null, unreadCount };
  });
