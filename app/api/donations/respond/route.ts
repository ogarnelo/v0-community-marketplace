  ...
  try {
    ...
    if (action === "accept") {
      ...
      if (conversationId) {
        await adminSupabase.from("messages").insert({
          ...
          sender_id: user.id,
          body: buildDonationChatBody({ requestId, status: "approved", note: donationRequest.note }),
        });
    ...
    if (conversationId) {
      await adminSupabase.from("messages").insert({
        conversation_id: conversationId,
        sender_id: user.id,
        body: buildDonationChatBody({ requestId, status: "rejected", note: donationRequest.note }),
      });
      await adminSupabase.from("conversations").update({ updated_at: now }).eq("id", conversationId);
    }
