...
export async function POST(request: Request) {
  try {
    ...
    const { data: listing, error: listingError } = await admin
      .from("listings")
      .select("id, seller_id, school_id, listing_type, type, price, status")
    ...
    const { data: existing } = await admin
      ...
      .in("status", ["proposed", "buyer_confirmed", "seller_confirmed", "confirmed", "disputed"])
      .order("created_at", { ascending: false })
      .limit(1)
      .maybeSingle();

    if (existing?.id) {
      return NextResponse.json({ ok: true, agreement: existing, alreadyExists: true });
    }
    ...
    const insertPayload: Record<string, unknown> = {
      ...
      amount: agreementType === "sale" ? listing.price : null,
      handoff_note: note,
    };
    ...
    if (insertError || !agreement) {
      return NextResponse.json({ error: insertError?.message || "No se pudo crear el acuerdo." }, { status: 500 });
    }

    await Promise.all([
      admin.from("agreement_events").insert({
    ...
    return NextResponse.json({ error: error?.message || "No se pudo proponer el acuerdo." }, { status: 500 });
  }
}
