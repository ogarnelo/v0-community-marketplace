...
const DISTANCE_OPTIONS = [
  { label: "1 km", value: 1 }, { label: "5 km", value: 5 }, { label: "10 km", value: 10 }, { label: "30 km", value: 30 },
  { label: "50 km", value: 50 }, { label: "100 km", value: 100 }, { label: "200 km", value: 200 }, { label: "+200", value: Infinity },
];
...
export function MarketplaceClient({ initialListings, initialSchoolId, initialPostalCode = "" }: MarketplaceClientProps) {
  ...
  const minPrice = priceRange[0] ?? 0; const maxPrice = priceRange[1] ?? PRICE_LIMIT; const selectedDistance = DISTANCE_OPTIONS[distanceIndex] ?? DISTANCE_OPTIONS[DISTANCE_OPTIONS.length - 1]; const maxDistanceKm = selectedDistance.value; const hasViewerPostalCode = Boolean(normalizePostalCode(viewerPostalCode));
  ...
  const filteredListings = useMemo(() => {
    ...
    const filtered = listingsWithDistance.filter((l) => {
      ...
      if (normalizedSearch) { const matchesSearch = normalizeText(l.title).includes(normalizedSearch) || normalizeText(l.description || "").includes(normalizedSearch) || normalizeText(l.category || "").includes(normalizedSearch) || normalizeText(l.gradeLevel || "").includes(normalizedSearch) || normalizeText(l.isbn || "").includes(normalizedSearch); if (!matchesSearch) return false; }
  ...
  useEffect(() => { setSaveSearchStatus("idle"); }, [searchQuery, isbnQuery, category, gradeLevel, listingType, condition, onlyMyCommunity, priceRange, distanceIndex, publishedDateFilter]);
  ...
  const clearFilters = () => { setOnlyMyCommunity(false); setCategory("all"); setGradeLevel("all"); setListingType("all"); setCondition("all"); setSearchQuery(""); setIsbnQuery(""); setPriceRange([0, PRICE_LIMIT]); setDistanceIndex(DISTANCE_OPTIONS.length - 1); setPublishedDateFilter("all"); };
  const saveSearch = async () => { if (!hasSearchIntent || saveSearchStatus === "saving" || saveSearchStatus === "saved") return; setSaveSearchStatus("saving"); try { const response = await fetch("/api/marketplace/saved-searches", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ query: searchQuery.trim() || null, isbnQuery: isbnQuery.trim() || null, category: category === "all" ? null : category, gradeLevel: gradeLevel === "all" ? null : gradeLevel, listingType: listingType === "all" ? null : listingType, condition: condition === "all" ? null : condition, onlyMyCommunity, radiusKm: maxDistanceKm === Infinity ? null : maxDistanceKm, resultsCount: filteredListings.length, sourcePath: "/marketplace" }) }); if (response.status === 401) return setSaveSearchStatus("auth"); if (!response.ok) throw new Error("saved_search_failed"); setSaveSearchStatus("saved"); } catch { setSaveSearchStatus("error"); } };
  ...
  const FilterControls = () => (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-3"><div className="flex items-center justify-between gap-3"><Label className="text-sm font-medium text-foreground">Precio</Label><span className="text-xs font-medium text-muted-foreground">{minPrice} € - {maxPrice >= PRICE_LIMIT ? "+200 €" : `${maxPrice} €`}</span></div><Slider min={0} max={PRICE_LIMIT} step={5} value={priceRange} onValueChange={(value) => setPriceRange([value[0] ?? 0, value[1] ?? PRICE_LIMIT])} minStepsBetweenThumbs={1} aria-label="Rango de precio" className="py-3" /></div>
      <div className="flex flex-col gap-3 rounded-2xl border border-border bg-card p-3"><div className="flex items-center justify-between gap-3"><Label className="flex items-center gap-2 text-sm font-medium text-foreground"><MapPin className="h-4 w-4 text-primary" />Distancia máxima</Label><span className="text-xs font-medium text-muted-foreground">{selectedDistance.label}</span></div><Slider min={0} max={DISTANCE_OPTIONS.length - 1} step={1} value={[distanceIndex]} onValueChange={(value) => setDistanceIndex(value[0] ?? DISTANCE_OPTIONS.length - 1)} disabled={onlyMyCommunity} aria-label="Distancia máxima" className="py-3" /><div className="flex justify-between text-[10px] text-muted-foreground"><span>1 km</span><span>50 km</span><span>+200</span></div><p className="text-xs leading-relaxed text-muted-foreground">{onlyMyCommunity ? "Al usar Solo mi comunidad, la distancia queda desactivada." : hasViewerPostalCode ? "Usamos tu código postal como zona aproximada. No mostramos direcciones exactas." : "Añade tu código postal en Mi cuenta para mejorar la distancia aproximada."}</p></div>
