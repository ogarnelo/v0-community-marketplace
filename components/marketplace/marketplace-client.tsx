...
export function MarketplaceClient({ initialListings, initialSchoolId, initialPostalCode = "" }: MarketplaceClientProps) {
  ...
  useEffect(() => { setSaveSearchStatus("idle"); }, [searchQuery, isbnQuery, category, gradeLevel, listingType, condition, onlyMyCommunity, priceRange, distanceKm, publishedDateFilter]);
  ...
  const clearFilters = () => { setOnlyMyCommunity(false); setCategory("all"); setGradeLevel("all"); setListingType("all"); setCondition("all"); setSearchQuery(""); setIsbnQuery(""); setPriceRange([0, PRICE_LIMIT]); setDistanceKm(DISTANCE_MAX); setPublishedDateFilter("all"); };
