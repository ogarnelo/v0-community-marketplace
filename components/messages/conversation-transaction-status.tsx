...
import ReportTransactionIssueButton from "@/components/issues/report-transaction-issue-button";
        ...
        <CardContent className="space-y-3 p-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-muted-foreground">{amount !== null ? `Importe acordado: ${formatPrice(amount)}` : "Importe acordado en el chat"}</p>
            </div>
            <Badge variant="outline" className={getStatusClass(payment?.status || offer.status)}>{getStatusLabel(payment, offer)}</Badge>
          </div>

          {payment?.status === "paid" ? (
            <div className="space-y-3 rounded-xl bg-slate-50 p-3 text-sm text-slate-700">
              <p>{shippingSelected ? "Pago confirmado. El siguiente paso es preparar el envío y compartir el seguimiento cuando esté disponible." : "Pago confirmado. Ya podéis cerrar la entrega en mano desde este chat."}</p>
              <div className="flex flex-wrap gap-2">
                <Link href="/account/activity"><Button variant="outline" size="sm">Ver actividad</Button></Link>
                {payment.id ? <ReportTransactionIssueButton paymentIntentId={payment.id} /> : null}
              </div>
            </div>
          ) : buyerCanPay ? (
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-amber-50 p-3">
              <div className="text-sm text-amber-800">La oferta ya está aceptada. Solo falta completar el pago dentro de Wetudy para cerrar la compra.</div>
              <Link href={`/checkout/${offer.id}`}><Button className="bg-[#7EBA28] text-white hover:bg-[#6da122]">Ir al pago</Button></Link>
            </div>
          ) : payment?.status === "failed" || payment?.status === "cancelled" ? (
            <div className="rounded-xl bg-rose-50 p-3 text-sm text-rose-700">El intento de pago no se completó. El comprador puede volver a intentarlo desde el checkout.</div>
          ) : null}
        </CardContent>
      </Card>
      {shippingSelected && payment?.status === "paid" && shipment ? <ShipmentStatusCard shipment={shipment} currentUserId={currentUserId} compact /> : null}
    </div>
  );
}
